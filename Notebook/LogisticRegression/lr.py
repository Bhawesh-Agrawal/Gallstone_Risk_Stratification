...
if __name__ == "__main__":
    ...
    #lr = train_base_lr(X_train, y_train)
    #metrics = evaluate_lr(X_test, y_test, lr, results_dir, "LR_V2")
    #save_metrics(metrics, metrics_file, "LR_V2")

    # ORIGINAL DATASET TRAINING (Grid Search)
    #X_train, X_test, y_train, y_test = original_dataset(data)
    #best_model, best_params, best_score = train_lr_gridsearch(X_train, y_train)
    #metrics = evaluate_lr(X_test, y_test, best_model, plot_dir=results_dir, model_name="LR_V3")
    #metrics["best_params"] = best_params
    #metrics["best_cv_score"] = best_score
    #save_metrics(metrics, metrics_file, "LR_V3")

    # FEATURED DATASET TRAINING (Grid Search)
    X_train, X_test, y_train, y_test = featured_dataset(data)
    best_model, best_params, best_score = train_lr_gridsearch(X_train, y_train)
    metrics = evaluate_lr(X_test, y_test, best_model, plot_dir=results_dir, model_name="LR_V4")
    metrics["best_params"] = best_params
    metrics["best_cv_score"] = best_score
    save_metrics(metrics, metrics_file, "LR_V4")
